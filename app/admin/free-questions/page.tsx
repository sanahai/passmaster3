...
export default async function AdminFreeQuestionsPage({
  searchParams,
}: {
  ...
  // 과정별 무료체험 문제 수 (목표 100문제 대비)
  const freeCounts = await prisma.question.groupBy({
    by: ["courseId"],
    where: { isFree: true },
    _count: { _all: true },
  });
  const freeCountByCourse = new Map(freeCounts.map((g) => [g.courseId, g._count._all]));

  const where: Prisma.QuestionWhereInput = { courseId, isFree: true };
  if (query) {
  ...
  const courseFreeTotal = freeCountByCourse.get(courseId) ?? 0;
  ...
  const buildHref = (opts: { course?: number; page?: number; q?: string }) => {
    ...
    return `/admin/free-questions?${params.toString()}`;
  };

  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-beauty-neutral">🎁 무료체험 문제 관리</h1>
        <Link href="/admin/free-questions/upload" className="btn-outline px-4 py-2 text-sm">
          🆓 무료체험 일괄 업로드
        </Link>
      </div>
      <p className="mb-6 text-sm text-beauty-gray">
        각 자격증별 무료체험용 문제(목표 100문제)를 등록·관리합니다. 여기에 등록되는 문제는 모두
        무료(무료체험)로 표시됩니다.
      </p>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {courses.map((c) => {
          const cnt = freeCountByCourse.get(c.id) ?? 0;
          return (
            <Link
              key={c.id}
            ...
            >
              {c.name}{" "}
              <span className={c.id === courseId ? "text-white/80" : "text-beauty-gray"}>
                ({cnt}/{TARGET})
              </span>
            </Link>
          );
      ...
      <div className="card mb-6">
        <div className="mb-1 flex justify-between text-sm">
          ...
          <span className="font-bold text-primary">
            {courseFreeTotal} / {TARGET}문제
          </span>
        </div>
        <div className="h-2.5 w-full overflow-hidden rounded-full bg-primary-pale">
          <div
            className="h-full rounded-full bg-primary"
            style={{ width: `${Math.min(100, (courseFreeTotal / TARGET) * 100)}%` }}
          />
        </div>
      </div>

      {/* 검색 */}
      <form method="get" action="/admin/free-questions" className="mb-6 flex gap-2">
      ...
      <details className="card mb-6">
        <summary className="cursor-pointer font-bold text-primary">+ 새 무료체험 문제 등록</summary>
        <form action={createQuestionAction} className="mt-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex items-center text-sm font-semibold text-beauty-success">
              🎁 무료체험 문제로 등록됩니다
            </div>
          </div>
      ...
      <p className="mb-2 text-sm text-beauty-gray">
        {query ? (
          ...
          <>총 {total.toLocaleString()}문제 (무료체험)</>
        )}
      </p>
      {total === 0 && (
        <div className="card text-center text-beauty-gray">
          {query ? "검색 결과가 없습니다." : "등록된 무료체험 문제가 없습니다. 일괄 업로드로 100문제를 등록해 보세요."}
        </div>
      )}
